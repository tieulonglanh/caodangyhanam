 ...
 */
class DocumentCategoryController extends XPHP_Controller
{
    ...
    #[Authorize]
    public function createPost(DocumentCategory $model)
    {
        if ($model->validate())
        {
    ...
    #[Authorize]
    public function editPost(DocumentCategory $model)
    {
        if ($model->validate())
        {
            if ($model->update())
    ...
    #[Authorize]
    public function deletePost(DocumentCategory $model)
    {
        if($model->delete())
            return $this->json(
                array('success' => true,
    ...
    private function _createHtmlCatMultiLevel($cats)
    {
        ...
        foreach ($cats as $cItem) {
            ...
            $html .= "<li id='list_{$cItem->id}'><a target='blank' href='/Document/Index/{$cItem->id}'>{$cItem->name}</a>
