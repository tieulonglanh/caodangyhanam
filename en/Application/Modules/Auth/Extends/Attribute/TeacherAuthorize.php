 ...
 * @author Mr.UBKey
 */
class Application_Modules_Auth_Extends_Attribute_TeacherAuthorize  extends XPHP_Action_Attribute_Abstract
{
    ...
    public function onActionExecute ()
    {
        ...
        //Kiểm tra theo csdl
        if(isset($session->teacher) && isset($session->role) && isset($session->permissions))
        {
            $valid = false;
            //Kiểm tra trong csdl
            ...
            if(! $valid)
                $url->redirectUrl($url->action("denied", "Teacher", "Auth", array($encodeUrl)));
        }
        else if(isset($session->teacher) && isset($session->role))
        {
            ...
            $url->redirectUrl($url->action("login", "Teacher", "Auth", array($encodeUrl)));
    }
