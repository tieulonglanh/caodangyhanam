 ...
 * @author Mr.UBKey
 */
class Application_Modules_Auth_Extends_Attribute_StudentAuthorize  extends XPHP_Action_Attribute_Abstract
{
    ...
    public function onActionExecute ()
    {
        ...
        //Kiểm tra theo csdl
        if(isset($session->student) && isset($session->role) && isset($session->permissions))
        {
            $valid = false;
            //Kiểm tra trong csdl
            ...
            if(! $valid)
                $url->redirectUrl($url->action("denied", "Student", "Auth", array($encodeUrl)));
        }
        else if(isset($session->student) && isset($session->role))
        {
            ...
            $url->redirectUrl($url->action("login", "Student", "Auth", array($encodeUrl)));
    }
