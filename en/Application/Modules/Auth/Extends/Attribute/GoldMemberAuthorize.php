 ...
 * @author Mr.UBKey
 */
class Application_Modules_Auth_Extends_Attribute_GoldMemberAuthorize  extends XPHP_Action_Attribute_Abstract
{
    ...
    public function onActionExecute ()
    {
        ...
        if(isset($session->member) && isset($session->mrole) && isset($session->mpermissions))
        {
            ...
            if(! $valid)
                $url->redirectUrl($url->action("golddenied", "Member", "Auth", array($encodeUrl)));
        }
