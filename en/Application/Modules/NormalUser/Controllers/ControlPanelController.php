...
class ControlPanelController extends XPHP_Controller {
    ...
    public function createAction() {
        ...
        $roleOptions = array();
        $roleOptions[1] = 'Giáo viên';
        $roleOptions[2] = 'Học sinh';
        $this->view->roleOptions = $roleOptions;

        return $this->view(new NormalUser());
    }
    #[Authorize]
    public function createPost(NormalUser $model) {
        if ($model->validate()) {
    ...
    public function editAction() {
        ...
        $id = $this->params[0];
        $model = new NormalUser($id);
        $model->password = "";
        return $this->view($model);
    }
    #[Authorize]
    public function editPost(NormalUser $model) {
        ...
        if ($model->validate()) {
            ...
            if ($model->status){
                $model->status = 1;
                    ...
                    $mail             = new PHPMailer();
                    $mail->CharSet = 'UTF-8';
                    $body             = "Chào {$model->fullname}, <br /> Tài khoản của bạn đã được kích hoạt trên cyd.edu.vn. Xin hãy vào đăng nhập vào website để sử dụng các dịch vụ!";
                    $mail->IsSMTP(); // telling the class to use SMTP
                    
                    $mail->SMTPAuth   = true;                  // enable SMTP authentication
                    $mail->Host       = "ssl://smtp.gmail.com"; // sets the SMTP server
                    $mail->Port       = 465;                    // set the SMTP port for the GMAIL server
                    $mail->Username   = "support@cyh.edu.vn"; // SMTP account username
                    $mail->Password   = "caodangyte123@";        // SMTP account password
                    $mail->SetFrom('support@cyh.edu.vn', 'Cao đẳng y tế Hà Nam');
                    $mail->AddReplyTo("support@cyh.edu.vn","Cao đẳng y tế Hà Nam");
                    $title = "Tài khoản của bạn đã được kích hoạt";
                    $mail->Subject = $title;
                    $mail->MsgHTML($body);
                    $address = $model->email;
                    $mail->AddAddress($address, "$model->fullname");
                    $mail->Send();
            }else
                $model->status = 0;
    ...
    #[Authorize]
    public function deletePost(NormalUser $model)
    {
        if($model->delete())
            return $this->json(
                array('success' => true,
    ...
    #[Authorize]
    public function lockPost(NormalUser $model) {
        $userModel = new User($model->id);
